...
## --- Palette --------------- ##
# RStudio and UI
chunk  <- "#DDE3D4"    # Chunk background,
page.1 <- "#E7EBE0"    # Page background, inactive tab background
page.2 <- "#D6DDCA"    # Frames: Toolbar, gutter and scroll bar background
page.3 <- "#C3CDB1"    # Window frames, selection, margin line, scroll handle
text.1 <- "#92A573"    # Inactive tab text
text.2 <- "#6B7C50"    # Gutter foreground, brackets
text.3 <- "#435030"    # Foregrounds: Code, text, Rstudio, and Rmd headings 
...
comment.1  <- "#627D36"   # Comments
operator.1 <- "#9540BF"   # Operators, data $, and YAML

## --- Create Theme --------------- ##
rsthemes::rstheme(
  theme_name = "~ Adeline",
  theme_path = "adeline.rstheme",
  ...
  ui_margin_line       = "$ui_rstudio_background",
